...
#define SAMPLING_FREQUENCY_KHZ 44.1f
#define AVERAGE_ENERGY_BUFFER_LEN 10
uint32_t interrupt_amount_for_energy_window = (SAMPLING_FREQUENCY_KHZ*1000)/(ADC_BUF_LEN/2);
...
int main(void)
{
  ...
  while (1)
  {
    if (adcDataReady)
    {
        ...
        int int_part = (int)total_energy;
        int frac_part = (int)((total_energy - int_part) * 1000); // 3 decimal digits

        len = sprintf(msg, "ADC avg: %d.%03d\r\n", int_part, frac_part);
        HAL_UART_Transmit(&huart2, (uint8_t*)msg, len, HAL_MAX_DELAY);

        int_part = (int)average_energy;
        frac_part = (int)((average_energy - int_part) * 1000); // 3 decimal digits

        len = sprintf(msg, "eng avg: %d.%03d\r\n", int_part, frac_part);
        HAL_UART_Transmit(&huart2, (uint8_t*)msg, len, HAL_MAX_DELAY);
    }
...
float calculate_discrete_energy(uint16_t *data, uint16_t length)
{
    ...
    for (int i = 0; i < length; i++) {
        float V = data[i] * Vref / ( (1 << ADC_bits) - 1 );
        energy += V * V * Ts;
    }
    if (interrupt_amount_for_energy_window_count == interrupt_amount_for_energy_window)
    {
      ...
      {
        float temp_energy_total = 0;
        for (int i = 0; i < AVERAGE_ENERGY_BUFFER_LEN; i++)
        {
          temp_energy_total+=average_energy_buffer[i];
        }
        average_energy = temp_energy_total/AVERAGE_ENERGY_BUFFER_LEN;
