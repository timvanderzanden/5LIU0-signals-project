...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (adcDataReady)
    {
        ...
        // Convert floating point values to int parts and fractional parts (3 decimal digits)
        int total_energy_int = (int)total_energy;
        int total_energy_frac = (int)((total_energy - total_energy_int) * 1000);

        int average_energy_int = (int)average_energy;
        int average_energy_frac = (int)((average_energy - average_energy_int) * 1000);

        int current_rms_int = (int)current_rms;
        int current_rms_frac = (int)((current_rms - current_rms_int) * 1000);

        int current_avg_voltage_int = (int)current_average_voltage;
        int current_avg_voltage_frac = (int)((current_average_voltage - current_avg_voltage_int) * 1000);

        // Create a single comma-separated message
        len = sprintf(msg, "%d.%03d,%d.%03d,%d.%03d,%d,%d,%d,%d.%03d\r\n",
                      total_energy_int, total_energy_frac,
                      average_energy_int, average_energy_frac,
                      current_rms_int, current_rms_frac,
                      top_bins[0], top_bins[1], top_bins[2],
                      current_avg_voltage_int, current_avg_voltage_frac);
